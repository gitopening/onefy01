...
//标题
$page->title = '';
//链接参数处理list_mtype_area_area2_(sprice-eprice)_room_(sta-eta)_page_关键字.html
...
$pageno = empty($param[6]) ? 1 : intval($param[6]);
$keywords = trim($param[7]);
...
}elseif ($orderby == 2 || empty($orderby)){
...
//缓存Key
$list_key = MEMCACHE_PREFIX . 'qgl_' . $cityInfo['id'] . '_' . $column['column_type'] . '_' . $mtype . '_' . $cityarea . '_' . $cityarea2 . '_' . $price . '_' . $room . '_' . $totalarea . '_' . $rent_type . '_' . $list_num . '_' . $pageno . '_' . $keywords;
$count_key = MEMCACHE_PREFIX . 'qgc_' . $cityInfo['id'] . '_' . $column['column_type'] . '_' . $mtype . '_' . $cityarea . '_' . $cityarea2 . '_' . $price . '_' . $room . '_' . $totalarea . '_' . $rent_type . '_' . $list_num . '_' . $keywords;
//从Memcache中取数据
...
if (empty($dataList) || empty($dataCount)) {
    ...
    $result = $Sphinx->Query($parsed_keywords, SPHINX_SEARCH_QIUGOU_INDEX);
    ...
    if ($result['total'] > 0) {
        ...
        foreach ($result['matches'] as $item) {
            $data = array();
            $data['title'] = get_new_house_title($item['attrs'], $cityarea_option, $cityarea2_list, $column['house_title'], 'qg');
...
//分页链接
$url = 'list_' . $mtype . '_' . $cityarea . '_' . $cityarea2 . '_' . $price . '_' . $room . '_' . $totalarea . '_{pageno}_' . $keywords . '.html';
...
switch ($_GET['column']){
	case 'qiugou':
        $page->title = '【'.$cityInfo['city_name'].$cityareaTitle.'求购二手房信息-个人求购二手房】-'.$page->titlec.$cityInfo['city_name'];
		$page->description = $cityInfo['city_name'].$page->titlec.'大量'.$cityInfo['city_name'].$cityarea_option[$cityarea]['region_name'].'求购二手房信息,个人求购二手房信息，最全面最及时的二手房求购信息尽在'.$page->titlec;
		$page->keywords = $cityInfo['city_name'].'求购二手房，'.$cityarea_option[$cityarea]['region_name'].'求购二手房、个人求购二手房、二手房求购';
		break;
	case 'xzlqg':
        $page->title = '【'.$cityInfo['city_name'].$cityareaTitle.'求购写字楼信息-个人求购写字楼】-'.$page->titlec.$cityInfo['city_name'];
        $page->description = $cityInfo['city_name'].$page->titlec.'大量'.$cityInfo['city_name'].$cityarea_option[$cityarea]['region_name'].'求购写字楼信息,个人求购写字楼信息，最全面最及时的写字楼求购信息尽在'.$page->titlec;
        $page->keywords = $cityInfo['city_name'].'求购写字楼，'.$cityarea_option[$cityarea]['region_name'].'求购写字楼、个人求购写字楼、写字楼求购';
		break;
	case 'spqg':
        $page->title = '【'.$cityInfo['city_name'].$cityareaTitle.'求购商铺信息-个人求购商铺】-'.$page->titlec.$cityInfo['city_name'];
        $page->description = $cityInfo['city_name'].$page->titlec.'大量'.$cityInfo['city_name'].$cityarea_option[$cityarea]['region_name'].'求购商铺信息,个人求购商铺信息，最全面最及时的商铺求购信息尽在'.$page->titlec;
        $page->keywords = $cityInfo['city_name'].'求购商铺，'.$cityarea_option[$cityarea]['region_name'].'求购商铺、个人求购商铺、商铺求购';
		break;
    case 'cwqg':
        $page->title = '【'.$cityInfo['city_name'].$cityareaTitle.'求购车位信息-个人求购车位】-'.$page->titlec.$cityInfo['city_name'];
        $page->description = $cityInfo['city_name'].$page->titlec.'大量'.$cityInfo['city_name'].$cityarea_option[$cityarea]['region_name'].'求购车位信息,个人求购车位信息，最全面最及时的车位求购信息尽在'.$page->titlec;
        $page->keywords = $cityInfo['city_name'].'求购车位，'.$cityarea_option[$cityarea]['region_name'].'求购车位、个人求购车位、车位求购';
        break;
    case 'cfqg':
        $page->title = '【'.$cityInfo['city_name'].$cityareaTitle.'求购厂房信息-个人求购厂房】-'.$page->titlec.$cityInfo['city_name'];
        $page->description = $cityInfo['city_name'].$page->titlec.'大量'.$cityInfo['city_name'].$cityarea_option[$cityarea]['region_name'].'求购厂房信息,个人求购厂房信息，最全面最及时的厂房求购信息尽在'.$page->titlec;
        $page->keywords = $cityInfo['city_name'].'求购厂房，'.$cityarea_option[$cityarea]['region_name'].'求购厂房、个人求购厂房、厂房求购';
        break;
    case 'ckqg':
        $page->title = '【'.$cityInfo['city_name'].$cityareaTitle.'求购仓库信息-个人求购仓库】-'.$page->titlec.$cityInfo['city_name'];
        $page->description = $cityInfo['city_name'].$page->titlec.'大量'.$cityInfo['city_name'].$cityarea_option[$cityarea]['region_name'].'求购仓库信息,个人求购仓库信息，最全面最及时的仓库求购信息尽在'.$page->titlec;
        $page->keywords = $cityInfo['city_name'].'求购仓库，'.$cityarea_option[$cityarea]['region_name'].'求购仓库、个人求购仓库、仓库求购';
        break;
    default:
...
//浏览过的房源
$history_list = get_history_list($_GET['column'] . 'house_qiugou');
...
foreach ($history_list as $item) {
    ...
    $memcache_house_key = 'house_qiugou_' . $house_id;
    ...
    $house_info = $house_query->field('house.*, extend.*')->table('qiugou', 'house')->join($house_query->db_prefix . 'qiugou_extend AS extend ON house.id = extend.house_id', 'LEFT')->where($condition)->cache($memcache_house_key, MEMCACHE_MAX_EXPIRETIME)->one();
    if (empty($house_info)) {
        continue;
    }
    $data = array();
    $data['title'] = get_new_house_title($house_info, $cityarea_option, $cityarea2_list, $column['house_title'], 'qg');
...
<body>
        ...
        <div class="header-search-box clear-fix">
            <div class="search-form">
                <form action="/qiugou/" method="get" id="search-form" onsubmit="return dosearch2();">
                    <input type="hidden" name="url" id="url" value="<?php echo 'list_'.$mtype.'_0_0_'.$price.'_'.$room.'_'.$totalarea.'_1_[keywords].html';?>" />
            ...
            <a href="/member/houseQiugou.php" target="_blank" class="publish-btn">发布房源</a>
            <div class="clear"></div>
        </div>
    </div>
    <div class="position list-position">
        ...
        if ($mtype == 1) {
            ...
            echo '<a href="list_3.html">' . $column['column_position_title'] . '非中介机构</a>';
        } elseif ($mtype == 4) {
        ...
        <div id="Region">
            <ul>
                ...
                <li>
                    ...
                    <div class="div02">
                        <?php
                        $url = 'list_'.$mtype.'_{curentparam}_0_'.$price.'_'.$room.'_'.$totalarea.'_1_.html';
                        if (empty($cityarea)){
                ...
                if ($cityarea > 0 && $cityarea2_option){
                    ...
                    <li class="Region2"><?php
                        $url = 'list_'.$mtype.'_'.$cityarea.'_{curentparam}_'.$price.'_'.$room.'_'.$totalarea.'_1_'.$keywords.'.html';
                        if (empty($cityarea2)){
                ...
                if ($show) {
                    ...
                    <li>
                        ...
                        <div class="div02">
                            <?php
                            $url = 'list_'.$mtype.'_'.$cityarea.'_'.$cityarea2.'_{curentparam}_'.$room.'_'.$totalarea.'_1_'.$keywords.'.html';
                ...
                if ($column['column_type'] == 4){
                    ...
                    <li>
                        ...
                        <div class="div02">
                            <?php
                            $url = 'list_'.$mtype.'_'.$cityarea.'_'.$cityarea2.'_'.$price.'_'.$room.'_{curentparam}_1_'.$keywords.'.html';
                ...
                <?php if ($column['column_type'] == 1){?>
                    <li>
                        ...
                        <div class="div02">
                            <?php
                            $url = 'list_'.$mtype.'_'.$cityarea.'_'.$cityarea2.'_'.$price.'_{curentparam}_'.$totalarea.'_1_'.$keywords.'.html';
                            if (empty($room)){
                                echo '<span><a href="'.build_url($url, 0).'">不限</a></span> ';
                            }else{
                                echo '<a href="'.build_url($url, 0).'">不限</a> ';
	...
	<div id="main" class="list-box house-normal-list">
        <div id="title01">
            <span <?php if (empty($mtype)){echo ' class="on"';}?>><a href="<?php echo 'list_0_'.$cityarea.'_'.$cityarea2.'_'.$price.'_'.$room.'_'.$totalarea.'_1_' . $keywords . '.html';?>">全部求购</a></span>
            <span <?php if ($mtype == 2){echo ' class="on"';}?>><a href="<?php echo 'list_2_'.$cityarea.'_'.$cityarea2.'_'.$price.'_'.$room.'_'.$totalarea.'_1_' . $keywords . '.html';?>">个人</a></span>
            <span <?php if ($mtype == 1){echo ' class="on"';}?>><a href="<?php echo 'list_1_'.$cityarea.'_'.$cityarea2.'_'.$price.'_'.$room.'_'.$totalarea.'_1_' . $keywords . '.html';?>">经纪人</a></span>
            <span <?php if ($mtype == 3){echo ' class="on"';}?>><a href="<?php echo 'list_3_'.$cityarea.'_'.$cityarea2.'_'.$price.'_'.$room.'_'.$totalarea.'_1_' . $keywords . '.html';?>">非中介机构</a></span>
            <?php
            if (!in_array($column['column_type'], array(4, 16, 17)) && $show_cooperation) {
            ?>
            <span <?php if ($mtype == 4){echo ' class="on"';}?>><a href="<?php echo 'list_4_'.$cityarea.'_'.$cityarea2.'_'.$price.'_'.$room.'_'.$totalarea.'_1_' . $keywords . '.html';?>">合作信息</a></span>
                <?php
            }
            ?>
            <span><?php
                        ...
                        <li>
                            ...
                            if ($column['column_type'] == 2 || $column['column_type'] == 3 || $column['column_type'] == 4){
                                ...
                                <div class="div07">
                                    ...
                                    } elseif ($item['end_totalarea'] > 0) {
                                        echo $item['start_totalarea'] . ' ㎡';
                                    } else {
										echo '不限';
									}
                                    ?>
                                ...
                                <div class="div08">
                                    ...
                                    if ($item['house_price'] == 0) {
                                        if ($item['start_price'] > 0 || $item['end_price'] > 0) {
                                            echo get_range_str($item['start_price'], $item['end_price'], ' 万元');
                                        } else {
