...
if ($id){
    ...
    $memcache_house_update_key = 'house_qiuzu_update_'.$id;
    if ($member_db == 'x') {
        ...
        $House = new HouseQiuzu($member_query);
    } elseif ($member_db == 4) {
...
$unit_type = ' ' . get_unit_type($dataInfo['unit_type']);  //价格单位
...
$dataInfo['article_title'] = get_new_house_title($dataInfo, $cityarea_option, $cityarea2_list, $column['house_title'], 'qz');
if (in_array($dataInfo['mtype'], array(1, 3, 4)) && $dataInfo['is_cooperation'] == 1) {
    $dataInfo['article_title'] .= '(合作)';
}

//浏览过的房源
$history_list = add_history_list($_GET['column'] . 'house_qiuzu', $dataInfo, $member_db);
...
if ($member_id) {
    ...
    $BrowsingHistory->Add($member_id, $db_index, 3, $dataInfo);
}

$page->title = '【'.$dataInfo['article_title'].'】'.$page->titlec.$cityInfo['city_name'];
$page->description = '最新最全大量' . $cityInfo['city_name'] . $dataInfo['cityarea_name'] . '求租' . $column['house_title'] . '信息,个人' . $column['house_title'] . '求租信息，找' . $column['house_title'] . '求租信息、发布求租' . $column['house_title'] . '信息就上'.$page->titlec;
$page->keywords = $dataInfo['cityarea_name'] . '求租' . $column['house_title'] . '信息、个人求租' . $column['house_title'] . '、' . $column['house_title'] . '求购信息';
...
/*
$mtype = 0;
$same_list_key = 'qzlist_' . $cityInfo['id'] . '_' . $column['column_type'] . '_' . $mtype . '_' . $dataInfo['cityarea_id'] . '_' . $dataInfo['cityarea2_id'] . '_' . $dataInfo['house_price'];
...
if (empty($same_house)) {
    ...
    $result = $sphinx->Query('', 'search_house_qiuzu,search_house_qiuzu_delta,member_house_qiuzu,member_house_qiuzu_delta');
    if ($result['total'] > 0){
        $house_id = array();
        ...
        if ($house_ids){
            $sql = "select h.id, h.house_room, h.house_price, h.start_price, h.end_price, h.unit_type, h.created, h.updated, h.cityarea_id, h.cityarea2_id, h.house_totalarea, h.start_totalarea, h.end_totalarea, h.mtype, h.rent_type, e.house_id, e.house_hall, e.house_toilet, e.house_title, e.house_address, e.borough_name, e.owner_phone from fke_qiuzu as h left join fke_qiuzu_extend as e on h.id = e.house_id where h.id in ($house_ids) and e.house_id in ($house_ids)";
        ...
        foreach ($result['matches'] as $item) {
            if ($item['attrs']['is_use_member_db'] == 1) {
                if($member_house_list[$item['id']]){
                    ...
                    $member_house_list[$item['id']]['title'] = get_new_house_title($member_house_list[$item['id']], $cityarea_option, $cityarea2_list, $column['house_title'], 'qz');
                ...
                if($house_list[$item['id']]){
                    ...
                    $house_list[$item['id']]['title'] = get_new_house_title($house_list[$item['id']], $cityarea_option, $cityarea2_list, $column['house_title'], 'qz');
    ...
    <div id="left">
        <div id="content">
            <div class="position">
                ...
                <a href="/<?php echo $_GET['column'];?>/"><?php echo $cityInfo['city_name'] . '求租' . $column['house_title'];?></a> &gt;
                <?php
                if ($dataInfo['cityarea_id']) {
                    ?>
                    <a href="list_<?php echo $dataInfo['mtype'];?>_<?php echo $dataInfo['cityarea_id'];?>.html"><?php echo $dataInfo['cityarea_name'] . '求租' . $column['house_title'];?></a> &gt;
                    <?php
                }
                if ($dataInfo['cityarea_id'] && $dataInfo['cityarea2_id']) {
                    ?>
                    <a href="list_<?php echo $dataInfo['mtype'];?>_<?php echo $dataInfo['cityarea_id'];?>_<?php echo $dataInfo['cityarea2_id'];?>.html"><?php echo $dataInfo['cityarea2_name'] . '求租' . $column['house_title'];?></a> &gt;
                    <?php
                }
            ...
            <h1><?php
                ...
                ?></h1>
            <div class="l_fy">
                <p>房源编号：qz<?php echo $dataInfo['id'];?> &nbsp;&nbsp;&nbsp;&nbsp;更新时间：<?php echo $dataInfo['updatetime'];?> &nbsp;&nbsp;&nbsp;&nbsp;<span id="click-number"></span></p>
                <?php
                        ...
                        <dd><?php
                            if ($dataInfo['house_price'] == 0) {
                                if ($dataInfo['start_price'] > 0 || $dataInfo['end_price'] > 0) {
                                    echo get_range_str($dataInfo['start_price'], $dataInfo['end_price'], $unit_type, '<strong class="price">{content}</strong>');
                                } else {
                                    echo '<strong class="price">面议</strong>';
                                }
                            } else {
                                echo '<strong class="price">' . $dataInfo['house_price'] . '</strong> ' . $unit_type;
                            }
                            ?></dd>
                    </dl>
                    <?php
                            ...
                            <div class="house-price"><?php
                                if ($house_price == 0) {
                                    if ($start_price > 0 || $end_price > 0) {
                                        echo get_range_str($start_price, $end_price, $unit_type, '<strong class="blues">{content}</strong>');
                                    } else {
                                        echo '<strong class="blues">面议</strong>';
                                    }
                                } else {
                                    echo '<strong class="blues">' . $house_price . '</strong> ' . $unit_type;
                                }
                                ?></div>
