...
class Image {
	...
	public function resizeImage($width, $height, $flag=1) {
		...
		if($flag == 5) {
			...
			//计算源图像截取的宽度和高度
			$dest_precent = $width / $height;
			$res_percent = $this->imageWidth / $this->imageHeight;
			if ($res_percent > $dest_precent) {
				$cutHeight = $this->imageHeight;
				$cutWidth = ceil($dest_precent * $this->imageHeight);
			} else {
				$cutWidth = $this->imageWidth;
				$cutHeight = ceil($this->imageWidth / $dest_precent);
			}
			imagecopyresampled($this->newResource, $this->imageResource, 0, 0, 0, 0, $width, $height, $cutWidth, $cutHeight);
		}else{
