...
{
    public $tName = 'house_new';
    public $tNameExtend = 'house_new_extend';
    public $tNameCheckLog = 'house_new_check_log';
    public $tNameMobleAuth = 'house_new_mobile_auth';
    public $tNamePic = 'house_new_pic';
    public $tNameShortData = 'house_new_short_data';
    public $tNamePromote = 'house_new_promote';
    public $tNameHomePromote = 'house_new_home_promote';
    public $tNameBannerPromote = 'house_new_banner_promote';
    public $tNameRefreshPlan = 'house_new_refresh_plan';
    ...
    public function saveHouse($fieldData)
    {
        ...
        if ($house_price > 0 && $house_totalarea > 0) {
            $house_price_average = round($house_price * 10000 / $house_totalarea, 2);
        } else {
        ...
        $field_array = array(
            ...
            'house_price_average' => $house_price_average,
    ...
    public function deleteHouse($ids, $member_id = 0)
    {
        ...
        if (!empty($nohouseid)) {
            ...
            //缓存更新
            update_house_memcache('new', $house_id);
            return true;
        } else {
            return false;
        }
        ...
        if ($result) {
            ...
            foreach ($ids as $id) {
                ...
                if ($data_info) {
                    ...
                    $sphinx_index = 'fyw_member_house_new_' . $index_name . ',fyw_member_house_new_delta,fyw_member_house_new_promote';
    ...
    */
    ...
    public function GetHouseURL($dataInfo, $suffix = '')
    {
        ...
        switch ($dataInfo['column_type']) {
            case 1:
                $url = $domain_url . '/zzx/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 2:
                $url = $domain_url . '/xzlx/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 3:
                $url = $domain_url . '/spx/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 4:
                $url = $domain_url . '/cwx/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 16:
                $url = $domain_url . '/cfx/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 17:
                $url = $domain_url . '/ckx/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            default:
                $url = '';
        }
