...
{
    public $tName = 'houserent';
    public $tNameExtend = 'houserent_extend';
    public $tNameCheckLog = 'houserent_check_log';
    public $tNameMobleAuth = 'houserent_mobile_auth';
    public $tNamePic = 'houserent_pic';
    public $tNameShortData = 'houserent_short_data';
    public $tNamePromote = 'houserent_promote';
    public $tNameHomePromote = 'houserent_home_promote';
    public $tNameBannerPromote = 'houserent_banner_promote';
    public $tNameRefreshPlan = 'house_rent_refresh_plan';
    ...
    public function saveHouse($fieldData)
    {
        ...
        $is_owner_direct_rent = intval($fieldData['is_owner_direct_rent']) == 1 ? 1 : 0;
        ...
        //价格转换成数字
        $fieldData['house_price'] = floatval($fieldData['house_price']);
        $fieldData['house_totalarea'] = floatval($fieldData['house_totalarea']);
        $fieldData['original_price'] = round($fieldData['house_price'], 2);

        if ($fieldData['house_type'] == 5 || $fieldData['house_type'] == 7) {
            //写字楼商铺前台搜索按元/平米/天
            if ($fieldData['unit_type'] == 2) {
                $house_month_price = $fieldData['house_price'] * $fieldData['house_totalarea'] * 30;
            } else {
                $house_month_price = $fieldData['house_price'];
                if ($fieldData['house_totalarea'] > 0) {
                    $fieldData['house_price'] = $fieldData['house_price'] / $fieldData['house_totalarea'] / 30;
                }
                $fieldData['unit_type'] = 2;
            }
        } elseif ($fieldData['house_type'] == 15) {
            //车位出租前台搜索单位换算成元/月
            if ($fieldData['unit_type'] == 2) {
        ...
        } elseif (in_array($fieldData['house_type'], array(16, 17))) {
            //出租前台搜索单位换算成元/月/平米，转让为元/月
            if ($fieldData['is_sublet'] == 1) {
                if ($fieldData['unit_type'] == 2) {
                    $fieldData['house_price'] = $fieldData['house_price'] * $fieldData['house_totalarea'] * 30;
                    $house_month_price = $fieldData['house_price'];
                } else {
                ...
                if ($fieldData['unit_type'] == 2) {
                    ...
                    $house_month_price = $fieldData['house_price'];
                    if ($fieldData['house_totalarea'] > 0) {
                        $fieldData['house_price'] = $fieldData['house_price'] / 30 / $fieldData['house_totalarea'];
                    } else {
                        $fieldData['house_price'] = 0;
                    }
                }
            }
        } else {
        ...
        //如果单位为元/㎡/天时，且价格大于100，则设置价格单位为 元/月
        if ($fieldData['unit_type'] == 2 && $fieldData['house_price'] > 100) {
            $fieldData['unit_type'] = 1;
            $house_month_price = $fieldData['house_price'];
        }
        ...
        if ($fieldData['rent_require']) {
            $fieldData['rent_require'] = ',' . implode(',', $fieldData['rent_require']) . ',';
        }
        if (!empty($fieldData['store_rent_type'])) {
            $fieldData['store_rent_type'] = implode(',', $fieldData['store_rent_type']);
        }
        ...
        $field_array = array(
            ...
            'house_month_price' => floatval($house_month_price),
            'unit_type' => intval($fieldData['unit_type']),
            'house_deposit' => intval($fieldData['house_deposit']),
            ...
            'qq' => $fieldData['qq'],
            'occupancy_time' => intval($fieldData['occupancy_time']),
            'is_sublet' => intval($fieldData['is_sublet']),
            ...
            'rent_require' => $fieldData['rent_require'],
            'brand_apartment_checked' => intval($fieldData['brand_apartment_checked']),
            'brand_apartment_id' => intval($fieldData['brand_apartment_id']),
            'parking_type' => intval($fieldData['parking_type']),
            'parking_pay_month' => intval($fieldData['parking_pay_month']),
            'parking_deposit_month' => intval($fieldData['parking_deposit_month']),
            'parking_lease_term' => intval($fieldData['parking_lease_term']),
            'transfer_fee' => floatval($fieldData['transfer_fee']),
            'first_floor_height' => intval($fieldData['first_floor_height']),
            'left_rent_month' => intval($fieldData['left_rent_month']),
            'lease_term' => intval($fieldData['parking_lease_term']),
            'floor_type' => intval($fieldData['floor_type']),
            'store_rent_type' => $fieldData['store_rent_type'],
            'commission' => intval($fieldData['commission']),
            'is_owner_direct_rent' => $is_owner_direct_rent,
    ...
    public function deleteHouse($ids, $member_id = 0)
    {
        ...
        if (!empty($nohouseid)) {
            ...
            //缓存更新
            update_house_memcache('rent', $house_id);
    ...
    public function updateHouse($ids, $fields_array)
    {
        ...
        if ($result) {
            //更新缓存
            update_house_memcache('rent', $ids);
            ...
            foreach ($ids as $id) {
                ...
                if ($data_info) {
                    ...
                    $sphinx_index = 'fyw_member_house_rent_' . $index_name . ',fyw_member_house_rent_delta,fyw_member_house_rent_promote';
    ...
    */
    ...
    public function GetHouseURL($dataInfo, $suffix = '')
    {
        ...
        switch ($dataInfo['column_type']) {
            case 1:
                $url = $domain_url . '/rent/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 2:
                $url = $domain_url . '/xzlcz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 3:
                $url = $domain_url . '/spcz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 4:
                if ($dataInfo['is_sublet'] == 1) {
                    $url = $domain_url . '/cwzr/house_' . $dataInfo['id'] . $suffix . '.html';
                } else {
                    $url = $domain_url . '/cwcz/house_' . $dataInfo['id'] . $suffix . '.html';
                }
                break;
            case 16:
                if ($dataInfo['is_sublet'] == 1) {
                    $url = $domain_url . '/cfzr/house_' . $dataInfo['id'] . $suffix . '.html';
                } else {
                    $url = $domain_url . '/cfcz/house_' . $dataInfo['id'] . $suffix . '.html';
                }
                break;
            case 17:
                if ($dataInfo['is_sublet'] == 1) {
                    $url = $domain_url . '/ckzr/house_' . $dataInfo['id'] . $suffix . '.html';
                } else {
                    $url = $domain_url . '/ckcz/house_' . $dataInfo['id'] . $suffix . '.html';
                }
                break;
            default:
                $url = '';
