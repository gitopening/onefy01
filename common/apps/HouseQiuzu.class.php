 ...
 * net geow@qq.com
 */
class HouseQiuzu extends Model
{
    public $tName = 'qiuzu';
    public $tNameExtend = 'qiuzu_extend';
    public $tNameCheckLog = 'qiuzu_check_log';
    public $tNameMobleAuth = 'qiuzu_mobile_auth';
    public $tNamePic = 'qiuzu_pic';
    public $tNameShortData = 'qiuzu_short_data';
    public $tNamePromote = 'qiuzu_promote';
    public $tNameRefreshPlan = 'house_qiuzu_refresh_plan';
    ...
    public function saveHouse($fieldData)
    {
        ...
        //价格转为数字
        $fieldData['start_price'] = floatval($fieldData['start_price']);
        $fieldData['end_price'] = floatval($fieldData['end_price']);
        //面积转为数字
        $fieldData['start_totalarea'] = floatval($fieldData['start_totalarea']);
        $fieldData['end_totalarea'] = floatval($fieldData['end_totalarea']);
        //月租金
        $house_month_price = 0;
        if ($fieldData['house_type'] == 5 || $fieldData['house_type'] == 7 || $fieldData['house_type'] == 15) {
            if ($fieldData['unit_type'] == 2) {
                $start_month_price = $fieldData['start_price'] * $fieldData['start_totalarea'] * 30;
                $end_month_price = $fieldData['end_price'] * $fieldData['end_totalarea'] * 30;
            } else {
                $start_month_price = $fieldData['start_price'];
                $end_month_price = $fieldData['end_price'];
            }
        } else {
            $start_month_price = $fieldData['start_price'];
            $end_month_price = $fieldData['end_price'];
        ...
        $field_array = array(
            ...
            'house_price' => 0,
            'start_price' => $fieldData['start_price'],
            'end_price' => $fieldData['end_price'],
            'house_month_price' => $house_month_price,
            'start_month_price' => $start_month_price,
            'end_month_price' => $end_month_price,
    ...
    public function deleteHouse($ids, $member_id = 0)
    {
        ...
        if (!empty($nohouseid)) {
            ...
            //缓存更新
            update_house_memcache('qiuzu', $house_id);
    ...
    public function updateHouse($ids, $fields_array)
    {
        ...
        if ($result) {
            //更新缓存
            update_house_memcache('qiuzu', $ids);
            ...
            foreach ($ids as $id) {
                ...
                if ($data_info) {
                    ...
                    $sphinx_index = 'fyw_member_house_qiuzu_' . $index_name . ',fyw_member_house_qiuzu_delta,fyw_member_house_qiuzu_promote';
    ...
    */
    ...
    public function GetHouseURL($dataInfo, $suffix = '')
    {
        ...
        switch ($dataInfo['column_type']) {
            case 1:
                $url = $domain_url . '/qiuzu/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 2:
                $url = $domain_url . '/xzlqz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 3:
                $url = $domain_url . '/spqz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 4:
                $url = $domain_url . '/cwqz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 16:
                $url = $domain_url . '/cfqz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            case 17:
                $url = $domain_url . '/ckqz/house_' . $dataInfo['id'] . $suffix . '.html';
                break;
            default:
                $url = '';
        }
