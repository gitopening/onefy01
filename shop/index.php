...
$memcache_key = MEMCACHE_PREFIX . 'shop_' . $id . '_index_';
//店长二手房推荐列表
$brokerSaleList = $houseSell->field('id, mtype, is_cooperation, cityarea_id, cityarea2_id, city_website_id, column_type, house_price, house_totalarea, house_room, house_hall, house_toilet, house_toward, house_topfloor, house_floor, house_fitment, created, updated, house_title, borough_name, house_hall, house_toilet, elevator, parking_lot, bright_spot, house_thumb')->table($houseSell->tName)->where("mid='$id' and is_checked in (0,1) and is_delete = 0 AND is_down = 0 AND house_status IN (0,2,3,4,5)")->order('id DESC')->limit(7)->cache($memcache_key . 'sale', MEMCACHE_EXPIRETIME)->all();
foreach($brokerSaleList as $key=>$item){
    $brokerSaleList[$key]['url'] = $houseSell->GetHouseURL($item, 'x');
    $brokerSaleList[$key]['house_thumb'] = GetPictureUrl($item['house_thumb'], 2, MEMBER_DB_INDEX);
    $brokerSaleList[$key]['created'] = MyDate('Y-m-d H:i',$item['created']);
    $brokerSaleList[$key]['updated'] = time2Units(time()-$item['updated']);
    $brokerSaleList[$key]['house_price'] = FormatHousePrice($item['house_price']);
    $brokerSaleList[$key]['house_room'] = FormatHouseRoom($item['house_room']);
    if ($item['house_totalarea'] > 0 && $item['house_price'] > 0) {
        $brokerSaleList[$key]['house_price_average'] = round($item['house_price'] * 10000 / $item['house_totalarea'], 2);
    } else {
        $brokerSaleList[$key]['house_price_average'] = 0;
    }
    ...
    $brokerSaleList[$key]['is_promote'] = $house_promote['end_time'] > time() ? 1 : 0;
}
//店长出租房推荐列表
$brokerRentList = $houseRent->field('id, mtype, is_cooperation, cityarea_id, cityarea2_id, city_website_id, column_type, house_price, house_month_price, house_totalarea, house_room, created, updated, unit_type, rent_type, column_type, house_month_price, house_title, borough_name, house_hall, house_toilet, house_toward, house_topfloor, house_floor, house_fitment, is_sublet, transfer_fee, brand_apartment_checked, brand_apartment_id, elevator, parking_lot, bright_spot, rent_require, house_thumb')->table($houseRent->tName)->where("mid='$id' and is_checked in (0,1) and is_delete = 0 AND is_down = 0 AND house_status IN (0,2,3,4,5)")->order('id DESC')->limit(7)->cache($memcache_key . 'rent', MEMCACHE_EXPIRETIME)->all();
foreach($brokerRentList as $key=>$item){
    $brokerRentList[$key]['url'] = $houseRent->GetHouseURL($item, 'x');
    $brokerRentList[$key]['house_thumb'] = GetPictureUrl($item['house_thumb'], 2, MEMBER_DB_INDEX);
    $brokerRentList[$key]['created'] = MyDate('Y-m-d H:i',$item['created']);
    $brokerRentList[$key]['updated'] = time2Units(time()-$item['updated']);
    $brokerRentList[$key]['house_room'] = FormatHouseRoom($item['house_room']);
    //置顶信息
    ...
    $brokerRentList[$key]['is_promote'] = $house_promote['end_time'] > time() ? 1 : 0;
    //取得品牌公寓
    ...
    if ($item['column_type'] == 4 && $item['is_sublet'] == 1 && $item['transfer_fee'] > 0) {
        $brokerRentList[$key]['transfer_fee'] = floatval($item['transfer_fee']);
        $brokerRentList[$key]['is_sublet'] = intval($item['is_sublet']);
    }
    $brokerRentList[$key]['house_price'] = FormatHousePrice($item['house_price']);
    $brokerRentList[$key]['house_month_price'] = FormatHousePrice($item['house_month_price']);
}

$page->title = '【'.$middle_column.$dataInfo['realname'].'电话'.$dataInfo['mobile'].'】-'.$page->titlec.$cityInfo['city_name'];
        ...
        <div class="houselist list-box list-pic-text">
            <div id="list">
                ...
                <ul>
                    ...
                    foreach($brokerRentList as $item){
                        ?>
                        <li class="clear-fix">
                            <div class="list-house-thumb">
                                <a href="<?php echo $item['url'];?>" target="_blank"><img src="<?php echo !empty($item['house_thumb']) ? '/images/default_picture.jpg' : '/images/no_picture.jpg';?>" <?php echo !empty($item['house_thumb']) ? 'data-original="' . $item['house_thumb'] . '"' : '';?> /></a>
